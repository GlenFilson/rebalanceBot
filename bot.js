...
function printTrades(trades) {
  ...
  for (const trade of trades) {
    log(
      `${trade.from.padEnd(6)} ${trade.to.padEnd(6)} ${trade.fromAmount
        .toFixed(6)
        .padStart(15)} ${trade.toAmount.toFixed(6).padStart(15)} ${trade.amount
        .toFixed(2)
        .padStart(12)}`
    );
  }
...
async function getWalletBalances(walletAddress) {
  ...
  try {
    ...
    const solBalanceResponse = await axios.post(RPC_ENDPOINT, {
      jsonrpc: "2.0",
      id: 1,
      method: "getBalance",
    ...
    //log(`SOL balance response: ${JSON.stringify(solBalanceResponse.data, null, 2)}`);

    if (solBalanceResponse.data.result?.value !== undefined) {
      const solLamports = new Decimal(
        solBalanceResponse.data.result.value.toString()
      );
      //log(`SOL lamports: ${solLamports}`);
      balances["SOL"] = solLamports.div(
        new Decimal(10).pow(ASSETS["SOL"].decimals)
      );
      //log(`Converted SOL balance: ${balances['SOL']}`);
    } else {
      log("Warning: No SOL balance value found");
    }

    // Get token balances
    for (const [asset, details] of Object.entries(ASSETS)) {
      if (asset === "SOL") continue; // Skip SOL as we already handled it

      //log(`Fetching balance for ${asset} (mint: ${details.mint})...`);

      const tokenResponse = await axios.post(RPC_ENDPOINT, {
        jsonrpc: "2.0",
        id: 1,
      ...
      //log(`${asset} response: ${JSON.stringify(tokenResponse.data, null, 2)}`);
      ...
      if (tokenResponse.data.result?.value) {
        ...
        const totalBalance = tokenResponse.data.result.value.reduce(
          (sum, account) => {
            if (account.account.data.parsed?.info?.tokenAmount) {
              ...
              log(`Found amount: ${amount}`);
              return sum.add(new Decimal(amount.toString()));
            }
            return sum;
          },
          new Decimal("0")
        ...
        //log(`Total raw balance for ${asset}: ${totalBalance}`);

        // Convert to decimal representation
        balances[asset] = totalBalance.div(
          new Decimal(10).pow(details.decimals)
        );
        //log(`Converted balance for ${asset}: ${balances[asset]}`);
      } else {
        log(`No token accounts found for ${asset}`);
      }
    }
...
async function getPrices() {
  ...
  try {
    ...
    for (const item of data.parsed) {
      const token = Object.keys(TOKEN_IDS).find(
        (key) => TOKEN_IDS[key] === item.id
      );
      const price = new Decimal(item.price.price).mul(
        new Decimal(10).pow(item.price.expo)
      );
      prices[token] = price;
    }
...
function calculateRebalanceAmounts(balances, prices, totalValue) {
  ...
  for (const asset of Object.keys(ASSETS)) {
    ...
    rebalanceAmounts[asset] = rebalanceAmount.toDecimalPlaces(
      6,
      Decimal.ROUND_DOWN
    );
  }
...
function printSwaps(swaps) {
  ...
  for (const swap of swaps) {
    log(
      `${swap.from.padEnd(6)} ${swap.to.padEnd(6)} ${swap.amount
        .toFixed(6)
        .padStart(12)} ${swap.value.toFixed(2).padStart(12)}`
    );
  }
  log("-".repeat(40));
}
...
function printPortfolio(balances, prices, totalValue) {
  ...
  for (const asset of Object.keys(ASSETS)) {
    ...
    log(
      `${asset.padEnd(6)} ${balance.toFixed(3).padStart(12)} ${value
        .toFixed(2)
        .padStart(12)} ${allocation.toFixed(2).padStart(11)}% ${targetAllocation
        .toFixed(2)
        .padStart(8)}%`
    );
  }
  log("-".repeat(57));
  log(
    `${"Total".padEnd(6)} ${" ".repeat(12)} ${totalValue
      .toFixed(2)
      .padStart(12)} ${"100.00%".padStart(11)} ${"100.00%".padStart(8)}`
  );
}
...
async function getJupiterSwapInstructions(
  ...
  try {
    const quoteURL = `https://lite-api.jup.ag/swap/v1/quote?onlyDirectRoutes=true&inputMint=${inputMint}&outputMint=${outputMint}&amount=${amountLamports}&slippageBps=${slippageBps}`;
    const quoteResponse = await axios.get(quoteURL);

    const swapInstructionsURL =
...
async function createSwapTransaction(
  ...
  try {
    ...
    const amountLamports = amount
      .mul(new Decimal(10).pow(ASSETS[inputAsset].decimals))
      .toFixed(0);
    ...
    swapInstructionsResponse.computeBudgetInstructions.forEach(
      (instructionData) => {
    ...
    transaction.add(
      createTransactionInstruction(swapInstructionsResponse.swapInstruction)
    );
...
async function createTipTransaction(fromAccount, stashAmount = null) {
  ...
  try {
    ...
    if (stashAmount) {
      ...
      const stashTokenAmount = stashAmount
        .mul(new Decimal(10).pow(ASSETS["USDC"].decimals))
        .toFixed(0);
...
async function sendBundle(transactions) {
  ...
  try {
    ...
    const response = await axios.post(
      // "https://mainnet.block-engine.jito.wtf/api/v1/bundles", switched to below endpoint, Bill recommended
      "https://london.mainnet.block-engine.jito.wtf:443/api/v1/bundles",
      bundleData,
      {
...
async function createRebalanceTransactions(
  rebalanceAmounts,
  prices,
  fromAccount
) {
    ...
    if (asset !== "USDC" && amount.abs().gt(new Decimal("0.01"))) {
      if (amount.gt(0)) {
        ...
        swapTransactions.push(
          await createSwapTransaction(fromAccount, "USDC", asset, usdcAmount)
        );
      } else if (amount.lt(0)) {
        swapTransactions.push(
          await createSwapTransaction(fromAccount, asset, "USDC", amount.abs())
        );
      }
    }
  }
  ...
  try {
    ...
    while (true) {
      try {
        ...
        const { totalValue, usdcValue } = calculatePortfolioValue(
          balances,
          prices
        );
        ...
        if (initialPortfolioValue === null) {
          ...
          log(
            `Initialized initial portfolio value to: $${initialPortfolioValue.toFixed(
              2
            )}`
          );
        }
        ...
        // Calculate DELTA
        const delta = initialPortfolioValue
          ? totalValue.minus(initialPortfolioValue)
          : new Decimal(0);
        log(`Current DELTA: $${delta.toFixed(2)}`);

        printPortfolio(balances, prices, totalValue);

        const rebalanceAmounts = calculateRebalanceAmounts(
          balances,
          prices,
          totalValue
        );
